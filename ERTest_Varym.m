...
for ip = 1:lt_p
    ...
    for im = 1:lt_m
    	...
    	for iter = 1:maxiter
            ...
            cost_greedy(ip,im) = cost_greedy(ip,im)+ c_greedy;
            cost_mcmc(ip,im) = cost_mcmc(ip,im)+mcmcalg(Mat(:,1:n),Mat(:,1:n),B_greedy); 
...
xlabel('Control input dimension m');
ylabel('Cost c');
